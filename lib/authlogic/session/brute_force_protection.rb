    ...
    module BruteForceProtection
      ...
      module Config
        ...
        def self.extended(klass)
          unless klass.class_variable_defined?('@@last_activity_field')
            klass.class_variable_set('@@last_activity_field', nil)
          end
        ...
        def last_activity_field(*field, &block)
          if field.empty?
            klass.class_variable_get('@@last_activity_field')
          else
            klass.class_variable_set('@@last_activity_field', field)
            klass.class_variable_set('@@last_activity_callback', &block)
          end
        end

        def last_activity_validate(record)
          # TODO: (@zerosign) should call this somewhere in this gem source code
          callback = klass.class_variable_get('@@last_activity_callback')
      ...
      module InstanceMethods
        ...
        def being_brute_force_protected?
          ...
          && (failed_login_ban_for <= 0 || (attempted_record.send(klass.last_activity_field) && attempted_record.updated_at >= failed_login_ban_for.seconds.ago)) \\
        end
